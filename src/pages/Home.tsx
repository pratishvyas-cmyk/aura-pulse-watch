...
export default function HomePage() {
  ...
  const readinessColor: "red" | "amber" | "gold" | "green" =
    readinessScore < 34 ? "red" :
    readinessScore < 60 ? "amber" :
    readinessScore < 80 ? "gold" :
    "green";
  ...
  return (
    <Screen className="space-y-6">
      ...
      <div className="rounded-2xl border border-subtle bg-surface p-4 shadow-card space-y-3">
        ...
        <StatBar label="Steps"    value={todaySteps}                   goal={STEPS_GOAL} color="green" />
        <StatBar label="Calories" value={todayCalories}                goal={CAL_GOAL}   unit="kcal" color="amber" />
        <StatBar label="Distance" value={Math.round(todayDistanceM)}   goal={DIST_GOAL}  unit="m"    color="blue" />
        <StatBar label="Active"   value={todayActiveMins}              goal={ACTIVE_GOAL} unit="min" color="red" />
      </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-teal">Last night's sleep</p>
      ...
      <div className="rounded-2xl border border-subtle bg-surface p-4 shadow-card">
        ...
        <ResponsiveContainer width="100%" height={60}>
          <LineChart data={hrData} margin={{ top: 4, right: 0, bottom: 0, left: -40 }}>
            ...
            />
            <Line type="monotone" dataKey="value" stroke="hsl(var(--status-red))" strokeWidth={2} dot={false} filter="url(#homeHrGlow)" />
          </LineChart>
        </ResponsiveContainer>
